...
class UserIdentity extends CUserIdentity
{
	...
	public function authenticate()
	{
		...
		if($user===null)
			...
			$this->setState('last_visit',$user->last_visit);
			$this->setState('superuser',$user->tipousuario);
			$this->setState('nombre',$user->nombre);			
			
			/*Actualizamos el last_login del usuario que se esta autenticando ($user->username) */
			$sql = "update frio_usuario set last_visit = now() where username='$user->username'";
